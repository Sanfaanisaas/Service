...
reporting.get('/dashboard/summary', asyncHandler(async (_req, res) => {
...
reporting.get('/history', asyncHandler(async (req, res) => {
  const config = await settings();
  ...
  const records = [
    ...charging.map((record) => ({ kind: 'charging', timestamp: record.createdAt, record })),
    ...workspace.map((record) => ({ kind: 'workspace', timestamp: record.createdAt, record })),
    ...sales.map((record) => ({ kind: 'sales', timestamp: record.createdAt, record })),
    ...transactions.map((record) => ({ kind: 'transactions', timestamp: record.createdAt, record })),
  ...
  res.json({ success: true, data: {
    date: start, summary: { charging: charging.length, workspace: workspace.length, sales: sales.length, revenue },
...
reporting.get('/search', asyncHandler(async (req, res) => {
