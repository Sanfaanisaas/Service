...
operations.get('/customers/:id', asyncHandler(async (req, res) => {
  if (req.authUser!.role === 'customer' && req.authUser!.customerId !== req.params.id) throw new ApiError(403, 'CUSTOMER_SCOPE_VIOLATION', 'You can only view your own customer record.');
...
operations.get('/charging/:id', asyncHandler(async (req, res) => {
  const charging = await ChargingSession.findById(req.params.id);
  if (!charging) throw new ApiError(404, 'CHARGING_NOT_FOUND', 'Charging session not found.');
  if (req.authUser!.role === 'customer' && charging.customerId.toString() !== req.authUser!.customerId) throw new ApiError(403, 'CUSTOMER_SCOPE_VIOLATION', 'You cannot view another customer\'s session.');
  res.json({ success: true, data: charging });
}));
operations.get('/customer/charging/:claimId', asyncHandler(async (req, res) => {
  const charging = await ChargingSession.findOne({ publicSessionId: req.params.claimId });
  if (!charging) throw new ApiError(404, 'CHARGING_NOT_FOUND', 'Charging session not found.');
  if (req.authUser!.role === 'customer' && charging.customerId.toString() !== req.authUser!.customerId) throw new ApiError(403, 'CUSTOMER_SCOPE_VIOLATION', 'You cannot view another customer\'s session.');
  res.json({ success: true, data: charging });
}));
...
operations.patch('/charging/:id/status', staff, asyncHandler(async (req, res) => {
  ...
  res.json({ success: true, data: await updateStatus(req.params.id, status, req.authUser!.id) });
}));
operations.post('/charging/:id/ready', staff, asyncHandler(async (req, res) => {
  res.json({ success: true, data: await updateStatus(req.params.id, 'ready', req.authUser!.id) });
}));
operations.post('/charging/:id/collect', staff, asyncHandler(async (req, res) => {
  ...
  res.json({ success: true, data: await collect(req.params.id, claimId, req.authUser!.id) });
}));
...
operations.post('/workspace/register', asyncHandler(async (req, res) => {
...
operations.post('/workspace/:id/check-in', staff, asyncHandler(async (req, res) => {
  res.json({ success: true, data: await checkInWorkspace(req.params.id, req.authUser!.id) });
}));
operations.post('/workspace/:id/check-out', staff, asyncHandler(async (req, res) => {
  res.json({ success: true, data: await checkOutWorkspace(req.params.id, req.authUser!.id) });
}));
...
operations.post('/products/:id/adjust', admin, asyncHandler(async (req, res) => {
  ...
  res.json({ success: true, data: await adjustStock(req.params.id, input.quantity, input.reason, req.authUser!.id) });
}));
...
operations.get('/receipts', asyncHandler(async (req, res) => {
  const filter = req.authUser!.role === 'customer' ? { customerId: req.authUser!.customerId } : {};
  res.json({ success: true, data: await Receipt.find(filter).sort({ generatedAt: -1 }).limit(200) });
}));
operations.get('/receipts/:id', asyncHandler(async (req, res) => {
  ...
  if (req.authUser!.role === 'customer' && receipt.customerId?.toString() !== req.authUser!.customerId) throw new ApiError(403, 'CUSTOMER_SCOPE_VIOLATION', 'You cannot view another customer\'s receipt.');
  res.json({ success: true, data: receipt });
}));

operations.get('/notifications', asyncHandler(async (req, res) => {
  const filter = req.authUser!.role === 'customer' ? { customerId: req.authUser!.customerId } : { userId: req.authUser!.appUserId };
  res.json({ success: true, data: await Notification.find(filter).sort({ createdAt: -1 }).limit(100) });
}));
operations.patch('/notifications/:id/read', asyncHandler(async (req, res) => {
  const filter = req.authUser!.role === 'customer' ? { _id: req.params.id, customerId: req.authUser!.customerId } : { _id: req.params.id, userId: req.authUser!.appUserId };
...
operations.patch('/staff/:id/role', admin, asyncHandler(async (req, res) => {
  ...
  const user = await AppUser.findByIdAndUpdate(req.params.id, { role }, { new: true });
  if (!user) throw new ApiError(404, 'USER_NOT_FOUND', 'User not found.');
  await audit(req.authUser!.id, 'USER_ROLE_UPDATED', 'user', user.id, { role });
