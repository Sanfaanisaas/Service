...
import { processPendingReceiptDeliveries } from './services/whatsapp.js';
...
reminderTimer.unref();
const receiptDeliveryTimer = setInterval(() => {
  void processPendingReceiptDeliveries().catch(() => {
    console.error('Receipt delivery processing failed.');
  });
}, 30_000);
receiptDeliveryTimer.unref();

void processPendingReceiptDeliveries().catch(() => {
  console.error('Initial receipt delivery processing failed.');
});
async function shutdown() {
  clearInterval(reminderTimer);
  clearInterval(receiptDeliveryTimer);
